function  [raw] = me2raw(echo_spacing,extbrain)
...
if num_pfile >= 10
    dummy = pfile(2:2+num_pfile-10);
    pfile(2:9) = pfile(3+num_pfile-10:num_pfile);
    pfile(10:num_pfile) = dummy;
end
...
%     options.fermiw = TE(ipfile)/(hdr.rdb.te*1e-6)*options.fermiw_first_echo;
%     Dim = [hdr.rdb.da_xres, hdr.rdb.user7, hdr.rdb.user8]; 
    ...
    % Extract brain if extbrain was defined 
    if exist('extbrain')
...
%     frq(:,:,:,ipfile) = CalcPhase(img,options)/TE(1)/2/pi;
    ...
    z = ceil(size(raw,3)/2);
    %subplot(211); 
    montageME(abs(raw),z,'1D'); title('Magnitude');
%     subplot(212); montageME(frq,z,'1D'); title('Frequency Map');
    if ipfile == 1
        writeNii(abs(img),'odd');        
    elseif ipfile == 2
        writeNii(abs(img),'even');
    end
    if ipfile < 10
        imgName = ['echo0' num2str(ipfile) '.raw.nii'];
    else
        imgName = ['echo' num2str(ipfile) '.raw.nii'];
    end
    nii = make_nii(img,vox,[0 0 0],16);
    save_nii(nii,imgName);    
end
...
% TE1 = TE(1);
fprintf('   Raw data reconstructed!\n');
end
