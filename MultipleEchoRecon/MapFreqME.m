function  [mag,frq,ang] = MapFreqME(echo_spacing,extbrain,monitor)
...
%     Dim = [hdr.rdb.da_xres, hdr.rdb.user7, hdr.rdb.user8]; 
    ...
    % Create unfiltered phase maps for BIAC cluster susceptibility map
    ang_map(:,:,:,ipfile) = angle(img);
    ...
    mag(:,:,:,ipfile) = abs(img);
    frq(:,:,:,ipfile) = CalcPhase(img,options)/TE(1)/2/pi;
    
    i=1000+ipfile;
    eval(sprintf('phase%d=ang_map(:,:,:,ipfile);',i));
    save(sprintf('phase%d.mat',i),sprintf('phase%d',i))
    eval(sprintf('mask%d=msk;',i));
    save(sprintf('mask%d.mat',i),sprintf('mask%d',i))
    
    % Redo unwrapping if desired, script continues if you wait too long
    unwrap_test = monitor;
    while strcmp(unwrap_test,'y') == 1
        figure(1989); 
        show(frq,z);
        unwrap_test = waitinput('   Redo unwrapping? (y/n): ',10,'s');
        if isnan(unwrap_test)
            fprintf('n\n');
        end
        if strcmp(unwrap_test,'y') == 1
            view_window = input(['   Input new view window (was ' ...
                                 num2str(view_window) '): ']);
            options.fermiw = TE(ipfile)/(hdr.rdb.te*1e-6)*8/512*view_window;
            frq(:,:,:,ipfile) = CalcPhase(img,options)/TE(1)/2/pi;            
        elseif ipfile < num_pfile
            monitor = waitinput('   Continue to monitor unwrapping? (y/n): ',5,'s');
            if isnan(monitor)
                fprintf('n\n');
            end
        end
    ...
    subplot(212); montageME(frq,z,'1D'); title('Frequency Map');
    if ipfile < 10
    ...
    frq_nii = make_nii(frq(:,:,:,ipfile),vox,[0 0 0],16);
...
% TE1 = TE(1);
save dims.mat dims
fprintf('   Frequency map complete!\n');
end
