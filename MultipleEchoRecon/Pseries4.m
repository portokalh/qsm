function  raw = Pseries4(varargin)
...
%   'shift'     - corrects even echo shift to match odd echoes
%   [-1 -1 -1]  - flips readout, phase1, and phase2 array orientations
%                 (good for getting data into WHS)
...
for narg = 1:nargin
    if ischar(varargin{narg})
        ...
        elseif strcmp(varargin{narg},'single')
            singleflag = 1;
        elseif strcmp(varargin{narg},'bin')
            binsave = 1;
        ...
        if size(varargin{narg}) == 3
            orientswitch = 1;
            echoshift = echoshift.*varargin{narg};
        end
    end   
end
...
databytes = dim1*dim2*dim3*dim4*2;
...
TE = (0:dim4-1)*echo_spacing+hdr.rdb.te; save TE TE
    ...
    for k = 1:dim4
        if s <= sets_pe
            ...
            for v = vstart:vstart+(views_pe-baseline_spacing*(s-1))-1; 
                ...
                dump=fread(fd,[2 dim1], pfile_type);
                raw(:,y,z,k)=squeeze(dump(2,1:end)+1i*dump(1,:));                   
                if (k == dim4 && exist('fillswitch','var'))
                    raw(:,y,z,k) = raw(:,y,z,k-1);
                end
            end
        end
...
% Reverse even echo alignment
if dim4 > 1
    fprintf('   Reversing even echoes...\n');
    raw(:,:,:,2:2:end) = raw(end:-1:1,:,:,2:2:end); %Consider block processing
end

% Save raw k-space
fprintf('   Saving raw k-space as raw.mat (short integer)...\n');
save raw raw
...
if exist('phaseswitch','var')
    fprintf('  Removing background phase...\n');    
    raw = phaseME(raw);
end
...
%     back = 0; time = 0;
%     for k = 1:dim4
%         [back,time] = progress(k,dim4,'Performing ifft on echo',back,time);
%         raw(:,:,:,k) = ifftnc(raw(:,:,:,k));
%     end
%     figure(1); montageME(abs(raw),ceil(dim3/2),'1D',dims(1:3));
    ...
    if exist('maskswitch','var')
        ...
        if exist('BIACswitch','var')
            msk = load_nii('msk.nii');
            for k = 1:dim4
                i = iBIAC+k-1;
                eval(sprintf('mask%d=msk.img;',i));
                save(sprintf('mask%d.mat',i),sprintf('mask%d',i))
                eval(sprintf('phase%d=single(angle(raw(:,:,:,%d)));',i,k));
...
if exist('binsave','var')
    fprintf('   Saving real and imaginary binary files...\n');
