...
% smj - 110519 Ripped out all the processing involving big arrays, moved it
%        to a C program. The program calcphase must be present in the
%        current directory.
%%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
%function [Freq] = MapFreq(Dim,pfile)
%function [Freq] = MapFreq(pfile)
function  MapFreqC(pfile)
%% parameters
if (~exist('pfile','var'))
    pfile = dir('PN*');
    if isempty(pfile)
...
for ipfile = 1:num_pfile
    ...
    % build up the system command to the C phase calculation program
    cmd = sprintf('%s %s %s %d %d %d %d %d %d %f %f %f %d',...
        './calcphase',name,FreqName,header_bytes,baseline_spacing,ptype,Dim(1),Dim(2),Dim(3),Dim(1)*fermir,Dim(1)*fermiw,TE,0);
    
    % call the C phase calculation program
