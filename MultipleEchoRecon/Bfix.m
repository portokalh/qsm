function [delB] = Bfix(raw,TE1,TE2)
...
img1 = CalcPhase(raw(:,:,:,1),options);
img2 = CalcPhase(raw(:,:,:,3),options);
...
% curve = fit((-50:49)',delB(79:178,xm,xm),'a*x+b','Startpoint',[0 0]);
...
for z = 1:v
    for y = 1:v
        q = q+1;
        curve = fit((xwin-ceil(m/2))',Hi(xwin,y,z),'a*x+b','Startpoint',[0 0]);
        slopev(q) = curve.a;
        yintv(q) = curve.b;
    end
end
...
while ((2*abs(slope-slope2)/abs(slope+slope2) > .05) && abs(slope) > .025) %abs(slope)?
    figure(1); dock; subplot(3,1,3); hist(slopev,w^2);
...
xm = 65;
figure(3); plot(-128:127,delB(:,xm,xm),-128:127,(-128:127)*slope+yint(2));
% 
