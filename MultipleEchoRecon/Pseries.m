function  [kspace] = Pseries(orientation)
...
for ipfile = 1:num_pfile
    ...
    fprintf(', %s %dx%dx%dx%d,', 'Dims:', dims(:));
    
    % Read pfile and align echoes properly
    kraw{ipfile} = (readepicCIVM(pfile(ipfile).name));
%     if (mod(ipfile,2) == 0)
...
datasize = filesize - header_bytes;
datapoints = datasize/bp_half_element;
nsets = datapoints/(baseline_spacing+1)/dim1/2; %Total for all echoes
nviews = nsets*(baseline_spacing); % Total for all echoes
...
raw=zeros(dim1,nviews/dim4,dim4);
for s = 1:ceil(nsets/dim4)
    for k = 1:dim4
        if s <= nsets/dim4
            for v = (s-1)*baseline_spacing+1:s*baseline_spacing    
                dump=fread(fd,[2 dim1], pfile_type);
                raw(:,v,k)=squeeze(dump(2,1:end)+1i*dump(1,:));
                if ( mod(v,baseline_spacing)==0)
                    skip=fread(fd,[2 dim1], pfile_type);  % skip other baselines
                end
            end
        else
            for v = (s-1)*baseline_spacing+1:nviews/dim4
                if k ~= dim4
                    dump=fread(fd,[2 dim1], pfile_type);
                    raw(:,v,k)=squeeze(dump(2,1:end)+1i*dump(1,:));                   
                else
