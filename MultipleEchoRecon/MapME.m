function  [mag,phs,raw] = MapME(echo_spacing,orientation,brainextract,idxstart)
...
% rmd - 110801 MapME - Save phase maps and masks as .mat files to be fed
%        into BIAC cluster to yield susceptibility maps.  Coregistration of
%        echoes performed after FSL transform matrix created
%%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++

%% parameters
% scan_run_num = pwd;
% scan_run_num = scan_run_num(end-5:end);
echoshift = [-2 0 0]; %pixel shift of max to aid in registration
xform = 'even2odd.mat';
if ~exist('brainextract','var')
    brainextract = 1;
end
...
for ipfile = 1:num_pfile
    ...
    if ipfile == 1
        ...
        dims = [hdr.rdb.da_xres, hdr.rdb.user7, hdr.rdb.user8]; save dims dims
        fov = [hdr.rdb.user16, hdr.rdb.user17, hdr.rdb.user18]; save fov fov
        vox = fov./dims; save vox vox   % voxel size, mm
        raw = zeros([dims num_pfile]);
    end
    
    % Read and display parameters, calculate fermi window size based on TE
    TE(ipfile)  = hdr.rdb.te*1.0e-6+echo_spacing*(ipfile-1); % us -> s
    options.fermiw = TE(ipfile)/TE(1)*8/512*view_window;
    fprintf(', %s %dx%dx%d, %s %g %s\n', 'Dims:', dims(:), 'TE:', TE(ipfile)*1000, 'ms');
    ...
    if strcmp(orientation,'180')
        img = img(end:-1:1,end:-1:1,:);
        if ipfile == 1
            echoshift = echoshift.*[-1 -1 1];
        end
    elseif strcmp(orientation,'vflip')
    elseif strcmp(orientation,'p13')
        img = permute(img,[3 2 1]);
        if ipfile == 1
            echoshift = permute(echoshift,[3 2 1]);
            raw = zeros([size(img) num_pfile]);
        end        
    end
    raw(:,:,:,ipfile) = img;  
end
...
fprintf('   Co-registering even and odd echoes...\n');
for ipfile = 1:num_pfile
    if mod(ipfile,2) == 0
        raw(:,:,:,ipfile) = registerM(raw(:,:,:,ipfile),xform,vox);
    end
    mag(:,:,:,ipfile) = abs(raw(:,:,:,ipfile)).*msk.odd;
    phs(:,:,:,ipfile) = angle(raw(:,:,:,ipfile));
    
    % Display
    figure(1);
    subplot(211); montageME(mag,z,'1D'); title('Coregistered Magnitude w/ Masking');
    subplot(212); montageME(phs,z,'1D'); title('Coregistered Phase');
    
    % Save .mat files for SusMapBIAC.m
    i = idxstart-1+ipfile;
    eval(sprintf('phase%d=phs(:,:,:,ipfile);',i));
    save(sprintf('phase%d.mat',i),sprintf('phase%d',i))
    eval(sprintf('mask%d=msk.odd;',i));
    save(sprintf('mask%d.mat',i),sprintf('mask%d',i))
end
...
fprintf('   Combining echoes for goodness of fit display...\n');
if num_pfile > 1
    figure(1985); 
%     undock(1985);
    combineME(mag);
...
% Save 4D data and 3D mask to be fed into BIAC (code under development by Wei)
fprintf('   Saving raw data for later use...\n');
r = num2str(idxstart-1);
r = str2num(r(1:2));
eval(sprintf('mask%d=msk.odd;',r));
save(sprintf('mask%d.mat',r),sprintf('mask%d',r))
eval(sprintf('phase%d=phs;',r));
save(sprintf('phase%d.mat',r),sprintf('phase%d',r))

fprintf('   Unfiltered phase map complete!\n');
end
