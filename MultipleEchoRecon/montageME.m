...
if ~exist('slice_num','var')
    slice_num = cslice(img);
end
...
if exist('f','var') == 1
    ...
    elseif strcmp(option,'1Dv') == 1
        montage(f,'DisplayRange',[fmin fmax],'Size',[size(f,4) 1]);
    elseif strcmp(option,'3') == 1
        montage(f,'DisplayRange',[fmin fmax],'Size',[3 size(f,4)/3]);
    elseif strcmp(option,'window') == 1
        montage(f,'DisplayRange',[fmin fmax/6]);
    elseif strcmp(option,'windowdown') == 1
        montage(f,'DisplayRange',[fmin*6 fmax*6],'Size',[1 size(f,4)]);
    ...
    elseif strcmp(option,'default')
        ncols = ceil(sqrt(dims(end)));
        nrows = ceil(dims(end)/ncols);
        montage(f,'DisplayRange',[fmin fmax],'Size',[nrows ncols]);
    end
    f = squeeze(f);
