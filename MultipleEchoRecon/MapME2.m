function  [mag,phs,raw] = MapME2(echo_spacing,orientation,brainextract,idxstart)
...
%         echoshift = round(dims(1)/128)*echoshift;
    end
...
if exist('orientation','var');
    echoshift = echoshift.*orientation;
...
mag.all = abs(raw);
mag.odd = mean(mag.all(:,:,:,1:2:end),4);
mag.evn = mean(mag.all(:,:,:,2:2:end),4);
