...
function  [mag,img4D] = openME(echo_spacing,orientation)
%% parameters
if (~exist('pfile','var'))
...
% Properly sorts the echo train files by taking echo #10 from position #2
% and putting it in position #10.
if num_pfile >= 10
    dummy = pfile(2);
...
for ipfile = 1:num_pfile
    ...
    if ipfile == 1
        img4D = zeros(hdr.rdb.da_xres,hdr.rdb.user7,hdr.rdb.user8,num_pfile);
        TE = zeros(1,num_pfile);
        z = ceil(size(img4D,3)/2);
    end
...
%     options.fermiw = TE(ipfile)/(hdr.rdb.te*1e-6)*options.fermiw_first_echo;
    ...
    if ~exist('orientation','var');
        figure(1);
        montageME(abs(img4D),z,'1D'); title('Magnitude');
    end

end
if exist('orientation','var') == 1
    if strcmp(orientation,'axi') ~= 1
        img4D = permute(img4D,[2 3 1 4]);
    end
    if strcmp(orientation,'cor') == 1
        img4D = imrotate(img4D(end:-1:1,:,:,:),-90);
    elseif strcmp(orientation,'sag');
        img4D = img4D(:,:,end:-1:1,:);
    end
    z = ceil(size(img4D,3)/2);
    figure(1);
    subplot(211); montageME(abs(img4D),z,'1D'); title('Magnitude');    
end
mag = abs(img4D);
% writeME(mag);
fprintf('   Magnitude reconstruction complete!\n');
end
