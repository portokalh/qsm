function Pconvert(scan_name)
%%function raw = Pconvert
% Before calling the function, put the pfile(s) (single- or multi-echo) in 
% a separate folder. Call the function in the created folder. They need to 
% be in alphabetical order (according to the MATLAB file folder window). 
%
% "scan_name" is an optional input.  If the function receives no input, the
% name of the output file will be based on the first p-file.
%
% Outputs array dimensions, field of view, voxel dimensions, the pfile
% header, and echo times as .mat files.
%
% Based on MapFreq.m, the function m-file created by Chunlei Liu
%++++++++++++++++++++++++++++++++++++++++++++++++++++++++
% ORIG Input:
...
%       data from multiple pfiles and sort it into echoes
%%+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
%     if isempty(pfile)
%         pfile = dir('*.rp'); % These already have baselines removed
%     end
    if isempty(pfile)
...
if ~exist('scan_name','var')
    scan_name = pfile(1).name;
end


%% Read in raw data

% P-file series parameters
[hdr,hdr_bytes] = WrapGEheader(pfile(1).name);
save hdr hdr hdr_bytes
...
if  ((hdr.rdb.user33 == 0) && dims(4) > 1)
    ...
    hdr.rdb.user33 = hdr.rdb.user33*1e3;
end
    
echo_spacing = hdr.rdb.user33/1e6; % Only valid in the PSD Russell uses
% if echo_spacing == 0
%     echo_spacing = 2896/1e6; % Assumption for pre-mge-PSD scans (256x128^2)
% end
TE = (0:dims(4)-1)*echo_spacing+hdr.rdb.te/1e6; save TE TE

% Display
fprintf('   %s %.0f x %.0f x %.0f x %.0f\n', 'Dimensions', dims(:));
fprintf('   %s %.3f x %.3f x %.3f x %.1f\n', 'Resolution (mm,ms):', ...
            vox(:), echo_spacing*1e3);

% Initialize
v = 0;
datatype = pfile_type;

fidvec = zeros(1,dims(4));
for k = 1:dims(4)
    fidvec(k) = fopen([scan_name 'k_' pfile_type '_echo_' num2str(k) '.raw'],'w');
end
...
[~,header_bytes] = WrapGEheader(filename);
...
sets_pe = datapoints/(baseline_spacing+1)/dims(1)/2/dims(4); % Sets per echo
...
skip=fread(fd,[2 dims(1)], pfile_type);                 % first baseline
back = 0; time = 0;
 
for s = 1:ceil(sets_pe)
    [back,time] = progress(s,ceil(sets_pe),'   Converting view set',back,time);
        vstart = v+1;
    for k = 1:dims(4)       
            if s <= sets_pe 
                for v = vstart:vstart+baseline_spacing-1               
                    dump = fread(fd,[2 dims(1)], pfile_type);
                    if mod(k,2) ~= 0
                        fwrite(fidvec(k),dump,datatype);
                    else
                        fwrite(fidvec(k),dump(:,end:-1:1),datatype);
                    end
                end 
            else
            ...
            skip = fread(fd,[2 dims(1)], pfile_type);  % skip other baselines   
    end
end
...
fclose all;
clear skip dump vstart views_pe sets_pe s ipfile datapoints 
clear SHORT_SIZE flag time back y z baseline_spacing 

fprintf('   P-file conversion complete!\n');
end
