...
% P-file read loop
for ee = 1:dim4
fprintf('   Reconstructing echo: %d/%d\n',ee,dim4);
% eval(sprintf('raw%d=zeros(dim1,dim2,dim3);',ee));




%PUT REPLACEMENT FUNCTION HERE
% Pass in dims
raw = Pseries0(pfile,ee);
...
%             end
%         else
%             for v = vstart:vstart+(views_pe-baseline_spacing)-1; 
%                 z = ceil(v/dim2);
%                 y = v-(z-1)*dim2;
% %                 if k ~= dim4
%                 dump=fread(fd,[2 dim1], pfile_type);
...
% eval(sprintf('raw%d=raw;',ee));
clear dump skip hdr
fprintf('   Saving raw k-space as raw%d.mat...\n',ee);
save(sprintf('raw%d.mat',ee),'raw')
clear raw;

end %end echo loop

clear vstart views_pe sets_pe s ipfile datapoints 
clear SHORT_SIZE flag time back y z baseline_spacing 

save TE TE

% % Correct echo alignment
% if dim4 > 1
...
% 
% % Save raw k-space
% fprintf('   Saving raw k-space as raw.mat...\n');
% save raw raw
% 
% % Perform IFFT
