function  [mag,phs,raw] = MapPhsME(echo_spacing,extbrain,idxstart,orientation)
...
echoshift = -2; %pixels
xform = ['even2odd_' scan_run_num '.mat'];
    ...
    if extbrain > 0
        if extbrain == 2    % Make two masks for odd and even echoes
            if ipfile == 1
                ...
                mag1 = msk1.*abs(img);
                mag1 = make_nii(mag1,vox,[0 0 0],16);
                save_nii(mag1,'echo1.nii');
            elseif ipfile == 2
...
%                     msk2 = stripMask(ref_nii,1);
                    msk2 = circshift(msk1,[echoshift 0 0]);
                end
                mag2 = msk2.*abs(img);
                mag2 = make_nii(mag2,vox,[0 0 0],16);
                save_nii(mag2,'echo2.nii');
                
                % Coregister .nii files w/help of FSL to get xform matrix
                if ~exist(xform,'file')
                    w = 5; count = 0;
%                     input(['   Hit enter when FSL matrix complete (' xform '):']);
                    while ~exist(xform,'file')
                        wstr = ['   Script has waited ' num2str(count,4) ...
                            ' seconds for FSL matrix, ' xform '.'];
                        fprintf('%s',wstr);
                        pause(w)
                        count = count + w;
                        for k = 1:length(wstr)
        ...
        mag(:,:,:,ipfile) = abs(msk.*raw(:,:,:,ipfile));
        phs(:,:,:,ipfile) = angle(raw(:,:,:,ipfile)); %no divide?
    end
    
    i=idxstart-1+ipfile;
    eval(sprintf('phase%d=phs(:,:,:,ipfile);',i));
    save(sprintf('phase%d.mat',i),sprintf('phase%d',i))
    eval(sprintf('mask%d=msk;',i));
...
r = str2num(r(1:2));
eval(sprintf('mask%d=msk;',r));
save(sprintf('mask%d.mat',r),sprintf('mask%d',r))
i = (i-idxstart+1)*100;
eval(sprintf('phase%d=phs;',i));
save(sprintf('phase%d.mat',i),sprintf('phase%d',i))
eval(sprintf('mask%d=msk;',i));
save(sprintf('mask%d.mat',i),sprintf('mask%d',i))
save dims.mat dims
