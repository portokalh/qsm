function img = RussReconAD(varargin)
...
%     back = 0; time = 0;
    ...
    %%% masking
        ...
        % debias weights
        [xx,~,zz] = ndgrid((-dims(1)/2:dims(1)/2-1)-ceil(dims(1)/20),1,dims(3):-1:1);
        w = repmat(xx.^2+1.33*zz.^2,[1 dims(2) 1]);
%         w2 = 1.0125.^(w/sqrt(dims(1)*dims(3)));
        w2 = 1.005.^(w/sqrt(dims(1)*dims(3)));
        mag1 = circshift(abs(mean(raw(:,:,:,1,:),5)),[-shift_vector(1) 0 0]).*w2;
%         show3(mag1);
    if (exist('mask0.nii.gz','file') ~= 2)&&(exist('mask0.nii','file') ~= 2)

        [mask,mag1thresh] = MakeMaskAD(mag1,1);
        while sum(mask(:)) > 8.5e5
            show3(mag1.*single(mask))
            drawnow
            [mask,mag1thresh] = MakeMaskAD(mag1,1,mag1thresh*1.05);
        end
        while sum(mask(:)) < 7.25e5
            show3(mag1.*single(mask))
            drawnow
            [mask,mag1thresh] = MakeMaskAD(mag1,1,mag1thresh*0.95);     
        end
        show3(mag1.*single(mask))
        drawnow
        mask = circshift(imerode(mask,strel3d_odd(3)),[shift_vector(1) 0 0]);
        save_nii(make_nii(uint8(mask),vox,[0 0 0],2),['mask0.nii']);
    else
        mask = open_nii('mask0.nii');
    end
    ...
    for k = 1:dims(4) % use 3*noise_floor for non BOMUS, 2 for BOMUS
        maskME(:,:,:,k) = imerode(imfill(single(mask).*abs(mean(raw(:,:,:,k,:),5)) > 3*noise_floor,'holes'),strel3d_odd(3)); 
    end
...
%                 [back,time] = progress(k,dims(4),'Combining coil data for echo',back,time);
                if dims(3) == 1
                    ...
                    if maskinflag
                        ...
                        img(:,:,:,k) = coil_combine_complex(squeeze(raw(:,:,:,k,:)),mask2);
                    else
